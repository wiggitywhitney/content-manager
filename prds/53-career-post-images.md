...
- [x] M9: Fix scripts for forward correctness (Decision 12, 15) — before running any further backfill operations, fix the scripts so they are safe and complete:
...
- [x] M14: Sync historical unsynced career posts to micro.blog as archive-only posts (Decision 16) — ~195 Video rows and potentially Podcast/Presentations rows exist in the spreadsheet but were never synced to micro.blog (Enlightning 2022–2024, You Choose! series, Cloud Native Live, IBM Cloud videos, and others). Implement `src/sync-historical-posts.js`:
  - Read both tabs of the production spreadsheet (`Sheet1` and `2024 & earlier`, same pattern as `backfill-career-images.js`); for each row where `row.microblogUrl` is empty AND `categoryForRow(row) !== null` (`categoryForRow` is exported from `restore-post-categories.js`), create a micro.blog archive post using `createMicroblogPost` from `sync-content.js` (with backdate and category)
  - **Archive-only — do NOT create a social post** (Decision 16): these are old posts; a "today's date" social post would flood the main feed with 100+ stale entries and cross-post them to LinkedIn/Bluesky/Mastodon
  - **DRY_RUN pattern**: use `process.argv.includes('--dry-run')` for the dry-run flag in this script (consistent with all other backfill scripts). Do NOT rely on `sync-content.js`'s module-level `DRY_RUN = (process.env.DRY_RUN ?? 'true')...` — that is a different mechanism used only for the daily sync and will default to dry-run if `DRY_RUN` env var is unset
  - Attach thumbnail images where `needsImage(row)` is true and a thumbnail is available; skip image gracefully on failure
  - Write the new post URL back to column H in the production spreadsheet (same as `writeUrlToSpreadsheet` in `sync-content.js`)
  - Include `--dry-run` flag and `[N/M]` progress indicators
  - Run with cross-posting disabled (creating posts updates the feed even without the social post)
  - After run, trigger a micro.blog site rebuild so category pages update
