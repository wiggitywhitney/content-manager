...
- [ ] M1: Visual verification gate — before writing any code, manually attach an image to one micro.blog career post via the micro.blog UI and confirm the category page rendering at whitneylee.com is visually acceptable. This is a go/no-go gate. If the layout is wrong, pause and reassess before continuing.
- [ ] M2: Image support added to `sync-content.js` for new career posts — in the post creation path only (do NOT modify the update/change-detection path), apply the image source rules table to determine whether to fetch an image; call `fetchThumbnail(row.link)` from `src/fetch-thumbnail.js` and attach via Micropub `photo[]`; posts without an image source are created as before with no `photo[]` field
- [ ] M3: Backfill script implemented — run `/research micropub update action photo` before writing any code to confirm the correct Micropub endpoint and action for adding a photo to an existing post; then implement `src/backfill-career-images.js` reading the staged spreadsheet, finding career posts with a micro.blog URL (column H) and an available image source, attaching the image via the researched Micropub update action, skipping posts already having an image, posts without an image source, and posts where image fetch fails (log warning, continue); include `--dry-run` flag and progress indicators
