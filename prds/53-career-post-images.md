...
| 2026-05-14 | `postHasPhoto` detection was broken during M5 backfill run — all 326 posts returned false, causing duplicate thumbnail uploads | micro.blog's Micropub source query (`?q=source&url=...`) likely doesn't return `photo` in the `properties` field in a format the current code expects; needs investigation of actual response shape before the fix can be written (M6) |
