...
- [ ] **M7: Existing rows backfilled and end-to-end verified** — The 8 pending short rows (15–18 and 23–26) predate this feature and have no Drive link. Backfill column O for each by running the updated journal skill or uploading videos to Drive manually and writing the file IDs. Row 15 ("Beyond Container Logs: Logging Operator's Extensions") is the highest priority — it has been blocked the longest. After backfill, run a dry-run to confirm row 15 is selected: `DRY_RUN=true vals exec -f .vals.yaml -- node src/post-social-content.js`. The log should show `[social] DRY_RUN: Would dispatch row 15 to [linkedin,bluesky,mastodon] (short): Beyond Container Logs...`. Confirm exit code 0 and no error logged. **Note**: dry-run exits before the Drive download call — it only confirms row selection. Actual Drive download is proven by M6's journal skill verification (the skill downloads from YouTube and uploads to Drive successfully, producing a valid file ID in column O). Do NOT trigger the GitHub Actions workflow to verify — that requires Whitney's explicit approval. Do NOT use `npm run sync:test` — that runs the career sync script, not social dispatch.
