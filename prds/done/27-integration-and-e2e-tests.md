...
- `gaxios` (googleapis HTTP transport) uses dynamic `import()` inside Jest's VM sandbox, which fails with "A dynamic import callback was invoked without --experimental-vm-modules". Fix: set `NODE_OPTIONS=--experimental-vm-modules` in the npm scripts and use separate Jest config files (jest.integration.config.js, jest.e2e.config.js) with `testEnvironment: 'node'`. The regular `npm test` (unit tests) is unaffected because it mocks googleapis.
- The Social Posts Queue may have existing pending rows when e2e tests run. The e2e test is designed to verify pipeline machinery (dispatch path, career guard check, DRY_RUN logging) regardless of which row is oldest. The test row serves as a safety net for when the queue is empty.
