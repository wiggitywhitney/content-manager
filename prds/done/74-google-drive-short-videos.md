...
- [x] **M8: Add `checkSocialPostedToday()` gate to prevent duplicate daily posts** — (Added per Decision Log 2026-06-04: social dispatch has no posted-today gate.) Add `checkSocialPostedToday()` to `src/social-posts-queue.js`. Model it on `checkCareerPostedToday()` in `src/career-post-guard.js` — same interface and error behavior. Read the `Social Posts Queue` tab using the `STAGED_SPREADSHEET_ID` constant already defined in `src/social-posts-queue.js` (do not add new config). Check columns G (scheduled date, YYYY-MM-DD) and I (status) for any row where `status === 'posted'` and the scheduled date starts with today's UTC date (`new Date().toISOString().slice(0, 10)`). Return `false` on any Sheets API error — do not throw. Export `checkSocialPostedToday` from `src/social-posts-queue.js`. Then update `post-social-content.js` at two call sites: (1) the social-priority skip block (~line 148, after the `checkCareerPostedToday()` call) — add an equivalent check; if social posted today, log `[social] Social content already posted today — skipping dispatch` and return; (2) the micro.blog scan gate (~line 199, where `careerPostedToday` gates `scanAndPostShorts`) — extend the condition to also skip the scan if social posted today, using `await checkSocialPostedToday().catch(() => false)`. The gate fires on both career-priority and social-priority days. Add tests: posted today → skip, not posted today → proceed, API error → proceed (returns false). All existing tests must pass.

- [x] **M7: End-to-end dry-run verification** — The original 8 pre-feature short rows mentioned in the PRD were already posted months ago and have since been removed from the queue. No backfill is needed. Run a dry-run to confirm the dispatch logic selects a pending short row correctly: `vals exec -f .vals.yaml -- bash -c 'DRY_RUN=true node src/post-social-content.js'`. The log should show a short row selected with its Drive file ID present (rows 19–22 are the current pending short rows with column O populated). Confirm exit code 0 and no error logged. **Note**: dry-run exits before the Drive download call — it only confirms row selection and that column O is read. Do NOT trigger the GitHub Actions workflow to verify — that requires Whitney's explicit approval. Do NOT use `npm run sync:test` — that runs the career sync script, not social dispatch.
...
| 2026-06-04 | Social dispatch needs a `checkSocialPostedToday()` gate | Discovered during M7: `post-social-content.js` has a career-posted-today gate but no equivalent for social posts. Manual posts or same-day re-runs cause duplicate posts. Fix: add `checkSocialPostedToday()` to `social-posts-queue.js` (reads Social Posts Queue for any row with `status=posted` and today's UTC date in column G); call it in `post-social-content.js` alongside the existing career gate — skip dispatch if either career or social posted today. Errors return `false` to avoid suppressing future posts. Tracked in separate GitHub issue. |
| 2026-06-04 | Bluesky `getServiceAuth` `aud` must be the user's PDS DID, not `did:web:video.bsky.app` | The video service saves the processed video to the user's PDS — the PDS is the intended audience. Discovered during M7 end-to-end verification (returned 401). Fix: derive from `agent.pdsUrl` after login: `` `did:web:${new URL(agent.pdsUrl).hostname}` ``. `agent.dispatchUrl` is undefined in `@atproto/api` ≥ 0.13.x and cannot be used. |

## Open Questions

- ~~What should happen when the journal skill tries to download a short that isn't yet public (private/scheduled)?~~ **Resolved (M5):** Leave column O empty — the cron skips-and-retries on future runs.
...
- M8 complete: `checkSocialPostedToday()` added to `src/social-posts-queue.js`; `post-social-content.js` updated at two call sites — dispatch skipped and `scanAndPostShorts` skipped when social already posted today; 10 new tests (469 total)
- M7 complete: end-to-end verified — row 19 dispatched to LinkedIn and Mastodon with Drive video; Bluesky had a pre-existing auth bug (wrong `aud` in `getServiceAuth` — was `did:web:video.bsky.app`, must be user's PDS DID); fixed and row 19 manually posted to Bluesky successfully
