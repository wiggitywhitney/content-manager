...
**Decision**: Replace Viktor's Telegram notifications with Slack, delivered via a single incoming webhook URL stored as a GSM secret — not a Slack bot token + `chat.postMessage`, and not an MCP server. The webhook URL is read only by a small fixed notifier script (e.g. `scripts/notify-slack.sh`) that this repo owns; it accepts nothing but one of the four defined event names (escalation, merge gate, run finished, worker-stuck) as an argument and does the `curl` itself. The orchestrator's prompt tells it *when* to call the script and with which event name — it never sees, holds, or is told the webhook URL itself, so the URL cannot appear in `.dot-agent-deck.toml`, in prompt text, in the orchestrator's own output, or in logs. The script enforces a 5-second connect timeout and 10-second total timeout, treats any non-2xx response as a failed send (logged, not raised as an error that blocks the run), and does not retry — a failed notification must never stall or block the deck. Before wiring the script into the orchestrator's config, send one manual test invocation to a real Slack channel to confirm delivery — no mocked-endpoint test is needed for a single fire-and-forget call with a response that isn't otherwise parsed.
**Rationale**: An incoming webhook is bound to one specific channel at creation time, satisfying Decision #5's "explicit destination id" requirement by construction. It is push-only with no inbound/updates side to read, satisfying the "never let the agent read the channel's inbound side" requirement without extra care. And because there's no MCP server involved at all, the "pin the MCP server's version" requirement doesn't apply. CodeRabbit correctly flagged that putting the webhook URL in the orchestrator's own config or prompt is a prompt-injection risk regardless of GSM storage: GSM only protects the secret at rest, not once it has entered model context, and the orchestrator's job is to read PRD files — untrusted-ish input a malicious or malformed PRD could exploit to get the model to leak the URL. A fixed wrapper script that takes only an event name closes that path entirely, and the timeout/non-2xx handling addresses the CodeRabbit finding that undefined `curl` failure behavior could stall the orchestrator.
