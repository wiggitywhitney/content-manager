...
**Decision**: Skip naming a separate low-stakes task for the dry-run milestone. Run PRD #125 (auto-adjusting posting cadence) directly through the deck as the first real run — validated entirely through the existing test suite (`npm test`, `npm run test:e2e`, which already control their own fixtures/dry-run state), never by any role directly invoking `sync-content.js`, `scan-new-content.js`, `post-social-content.js`, or `update-page-visibility.js` against production credentials or the live spreadsheets. If a role's plan calls for running one of these scripts directly as a manual check: `sync-content.js` and `post-social-content.js` must be invoked with `DRY_RUN=true` in their env (never omitted, since both default to live or near-live behavior); `scan-new-content.js` must be invoked with the `--dry-run` CLI flag specifically — its dry-run gate reads `process.argv`, not the `DRY_RUN` env var, so setting the env var alone is a no-op for this script; `update-page-visibility.js` has no dry-run mode of any kind and must not be invoked directly against production during this validation at all. Whitney reviews the run's output at the merge gate and decides separately, as an explicit follow-up action outside this PRD's automated run, whether and when to actually exercise the pipeline live. If the deck's config, prompts, or role handoffs turn out broken before reaching the merge gate, discard that branch without merging and fix the config — rather than spending a milestone validating on throwaway work first.
**Rationale**: Whitney's call, refined across two rounds of CodeRabbit findings. The first round flagged that `daily-sync.yml` forces `DRY_RUN=false` for the sync step and `post-social-content.js`'s workflow invocation doesn't set `DRY_RUN` at all. The second round went further: `DRY_RUN=true` isn't sufficient on its own, because (a) `scan-new-content.js` doesn't read the `DRY_RUN` env var at all — verified in `src/scan-new-content.js`, it only checks `args.includes("--dry-run")` — so an env-only guard would silently fail to prevent a real spreadsheet write from that script, and (b) `update-page-visibility.js` was verified to have no dry-run support of any kind — grepping the file finds no `DRY_RUN` or `dry-run` reference at all. Given that, the safer and simpler rule is that PRD #125's validation run doesn't invoke any of these production scripts directly in the first place — the existing test suite is what's meant to exercise this code path safely, and this repo's PRD workflow already runs it as a matter of course. This merges the "Dry run validated on a low-stakes task" milestone into "PRD #125 implemented through the deck" below — see the updated Milestones section.
