...
- Other confirmed `sandbox.*` keys (not exhaustive): `sandbox.filesystem.allowWrite`/`allowRead`/`denyWrite`/`denyRead`/`disabled`, `sandbox.credentials` (`files` + `mode: deny/allow`), `sandbox.allowAppleEvents` (macOS), `sandbox.enableWeakerNetworkIsolation`, `sandbox.excludedCommands`, `sandbox.allowUnixSockets`.
